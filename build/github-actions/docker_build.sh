...
then
  docker buildx build --platform $PLATFORM --load -t campsych/concerto-platform:$GITHUB_REF_NAME .
...
then
  ...
  docker buildx build --platform $PLATFORM --load -t campsych/concerto-platform:$COMMIT_SHORT .
...
then
  ...
  then
    docker buildx build --platform $PLATFORM --load -t campsych/concerto-platform:${BASH_REMATCH[1]}${BASH_REMATCH[2]} .
    docker tag campsych/concerto-platform:${BASH_REMATCH[1]}${BASH_REMATCH[2]}

    docker buildx build --platform $PLATFORM --load -t campsych/concerto-platform:${BASH_REMATCH[1]}${BASH_REMATCH[2]}.${BASH_REMATCH[3]} .
