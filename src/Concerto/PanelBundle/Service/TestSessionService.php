...
class TestSessionService
{
    ...
    public function __construct($environment, TestSessionRepository $testSessionRepository, TestRepository $testRepository, TestSessionLogRepository $testSessionLogRepository, $secret, LoggerInterface $logger, RRunnerService $rRunnerService, FileService $fileService, AdministrationService $administrationService, $testRunnerSettings, SessionInterface $session)
    {
    ...
    public function startNewSession($test_slug, $test_name, $params, $client_ip, $client_browser, $debug)
    {
        ...
        $rresult = $this->initiateTestNode($session->getHash(), $panel_node_port, $client_ip, $client_browser, $debug);
    ...
    public function backgroundWorker($session_hash, $values, $client_ip, $client_browser, $time)
    {
        ...
        $submitted = $this->submitToTestNode($test_node_port, $panel_node_port, $client_ip, $client_browser, $values, self::RESPONSE_WORKER);
        if (!$submitted) {
            return $this->prepareResponse($session_hash, array(
                "source" => self::SOURCE_PANEL_NODE,
                "code" => self::RESPONSE_SESSION_LOST
    ...
    public function keepAlive($session_hash, $client_ip, $client_browser)
    {
        ...
        $checked = $this->keepAliveTestNode($test_node_port, $client_ip, $client_browser);
        if (!$checked) {
    ...
    public function kill($session_hash, $client_ip, $client_browser)
    {
        ...
        $killed = $this->killTestNode($test_node_port, $client_ip, $client_browser);
        if (!$killed) {
    ...
    private function submitToTestNode($test_node_port, $panel_node_port, $client_ip, $client_browser, $values, $code = self::RESPONSE_SUBMIT)
    {
    ...
    private function keepAliveTestNode($test_node_port, $client_ip, $client_browser)
    {
        $this->logger->info(__CLASS__ . ":" . __FUNCTION__ . " - $test_node_port, $client_ip");
        if (($sock = socket_create(AF_INET, SOCK_STREAM, SOL_TCP)) === false) {
            return false;
        }
        if (socket_connect($sock, "127.0.0.1", $test_node_port) === false) {
    ...
    private function killTestNode($test_node_port, $client_ip, $client_browser)
    {
        $this->logger->info(__CLASS__ . ":" . __FUNCTION__ . " - $test_node_port, $client_ip, $client_browser");
        if (($sock = socket_create(AF_INET, SOCK_STREAM, SOL_TCP)) === false) {
            return false;
        }
        if (socket_connect($sock, "127.0.0.1", $test_node_port) === false) {
    ...
    private function initiateTestNode($session_hash, $panel_node_port, $client_ip, $client_browser, $debug, $values = null)
    {
        $this->logger->info(__CLASS__ . ":" . __FUNCTION__ . " - $session_hash, $panel_node_port, $client_ip, $client_browser, $debug, $values");

        return $this->rRunnerService->startR($panel_node_port, $session_hash, $values, $client_ip, $client_browser, $debug ? 1 : 0);
    }
