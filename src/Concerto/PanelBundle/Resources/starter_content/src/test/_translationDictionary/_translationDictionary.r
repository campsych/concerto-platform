...
CASE WHEN t1.{{languageColumn}} IS NULL THEN t2.{{defaultLanguageColumn}} ELSE t1.{{languageColumn}} END AS \"trans\"
FROM {{table}} AS t1
...
CASE WHEN {{languageColumn}} IS NULL THEN {{defaultLanguageColumn}} ELSE {{languageColumn}} END AS \"trans\"
